...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center {fullScreen ? 'p-0' : 'p-4'}">
		<button
		...
		></button>
		<div
			class="relative z-10 w-full overflow-y-auto bg-white shadow-xl {fullScreen
				? 'h-[100dvh] max-w-none rounded-none'
				: `max-h-[90vh] rounded-xl ${klass}`}"
		>
