<script lang="ts">
	...
	function saveReply() {
		...
		if (!body) return;
		// Log the client's reply on the conversation thread (the query stays in
		// Quoted; move it back to Working manually when they want changes).
		$addReply.mutate(
			{ query_id: q.id, body, sender: 'client', author: auth.user?.email ?? null },
			{ onSuccess: () => (draft = '') }
		);
	}
...
>
	...
	{#if expanded}
		<div class="border-t border-slate-100 px-3 py-2.5">
			...
			<!-- reply box: logs the client's reply on the conversation thread -->
			<textarea
				bind:value={draft}
				rows="2"
				placeholder="What did the client reply?"
