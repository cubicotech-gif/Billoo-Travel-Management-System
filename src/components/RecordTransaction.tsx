...
interface PassengerOption { id: string; first_name: string; last_name: string }
interface VendorOption { id: string; name: string }
interface InvoiceOption { id: string; invoice_number: string; amount: number; paid_amount: number }
...
export default function RecordTransaction({
  ...
  const [formData, setFormData] = useState({
    type: defaultType || 'payment_received',
    amount: '',
    currency: 'PKR',
  ...
  const loadOptions = async () => {
    const [pRes, vRes, iRes] = await Promise.all([
      ...
      supabase.from('invoices').select('id, invoice_number, amount, paid_amount').in('status', ['pending', 'partial', 'draft', 'sent']).order('created_at', { ascending: false }),
    ])
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="px-6 py-4 max-h-[calc(100vh-250px)] overflow-y-auto space-y-4">
              ...
              <div>
                ...
                <select
                  value={formData.type}
                  onChange={e => setFormData({ ...formData, type: e.target.value, passenger_id: '', vendor_id: '', invoice_id: '' })}
                  className="input"
                >
                  {TXN_TYPES.map(t => (
