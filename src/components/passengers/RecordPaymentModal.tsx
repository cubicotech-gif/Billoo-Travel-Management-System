...
export default function RecordPaymentModal({ passengerId, passengerName, onSuccess, onCancel }: RecordPaymentModalProps) {
                ...
                <select value={invoiceId} onChange={e => setInvoiceId(e.target.value)} className="input">
                  ...
                  {invoices.map(inv => (
                    <option key={inv.id} value={inv.id}>
                      {inv.invoice_number} — Due: {formatCurrency(inv.amount - inv.paid_amount)}
                    </option>
                  ))}
                </select>
              </div>
