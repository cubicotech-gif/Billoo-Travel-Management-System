...
<div class="mb-5 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-5">
	<Card><div class="text-xs uppercase tracking-wide text-slate-400">Collected</div><div class="mt-1 break-words text-lg font-bold sm:text-2xl text-green-600">{formatAmount(totalCollected, 'PKR')}</div></Card>
	<Card><div class="text-xs uppercase tracking-wide text-slate-400">Receivable</div><div class="mt-1 break-words text-lg font-bold sm:text-2xl text-brand-700">{formatAmount(totalReceivable, 'PKR')}</div></Card>
	<Card><div class="text-xs uppercase tracking-wide text-slate-400">Payable</div><div class="mt-1 break-words text-lg font-bold sm:text-2xl text-amber-600">{formatAmount(totalPayable, 'PKR')}</div></Card>
	<Card><div class="text-xs uppercase tracking-wide text-slate-400">Net position</div><div class="mt-1 break-words text-lg font-bold sm:text-2xl text-slate-800">{formatAmount(totalReceivable - totalPayable, 'PKR')}</div></Card>
	<Card><div class="text-xs uppercase tracking-wide text-slate-400">Net profit</div><div class="mt-1 break-words text-lg font-bold sm:text-2xl {($profit.data?.netProfit ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'}">{mask($profit.data?.netProfit ?? 0)}</div></Card>
</div>

<!-- Tab bar -->
<div class="-mx-4 mb-6 flex gap-1 overflow-x-auto whitespace-nowrap border-b border-slate-200 px-4 sm:mx-0 sm:flex-wrap sm:px-0">
	{#each TABS as t (t.id)}
		{@const Icon = t.icon}
		<button
			type="button"
...
{#if tab === 'overview'}
	<Card>
		<div class="mb-2 flex flex-wrap items-center justify-between gap-x-3 text-sm">
