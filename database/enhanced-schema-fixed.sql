...
-- This version avoids conflicts with existing triggers
...
-- Updated_at triggers (drop if exists to avoid conflicts)
...
BEGIN
  ...
  RAISE NOTICE '📊 Added 8 new tables: query_services, activities, documents, reminders, email_templates, communications, invoice_items, user_preferences';
  RAISE NOTICE '🔧 Added new columns to queries and vendors tables';
  RAISE NOTICE '📧 Inserted 4 email templates';
  RAISE NOTICE '🎉 Your database is now ready for all enhanced features!';
END $$;
